...
export default function PortfolioSection() {
  ...
  return (
    <section id="projects" className="section-spacing">
      <div
        className="relative h-105 overflow-hidden rounded-[28px] bg-[var(--bg-secondary)]"
        style={{
          backgroundImage:
        ...
        <Reveal>
          <div className="mt-10 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {t.portfolio.cards.map((project) => (
              <div
                key={project.title}
                className="group relative h-full overflow-hidden rounded-3xl border border-white/10 bg-[var(--card)] p-6 shadow-soft transition-all duration-300 hover:-translate-y-2 hover:border-white/20 hover:shadow-[0_30px_80px_-50px_rgba(0,130,251,0.5)]"
              >
